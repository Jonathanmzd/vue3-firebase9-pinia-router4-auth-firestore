    ...
    <div>
        ...
        <a-space direction="vertical" v-if="!databaseStore.loadingDoc" style="width: 100%">
            <a-card v-for="item of databaseStore.documents" :key="item.id" :title="item.short">
                <template #extra>
                    <a-space>
                        <a-popconfirm title="Esta seguro que quiere Eliminar?" ok-text="Si" cancel-text="No"
...
const confirm = async (id) => {
    ...
    if (!error) { return message.success('Se elimina Correctamente') }
    return message.error(error)
}

const cancel = () => {
    message.success('No se elimina')
}
